...
For convenience we are building a Fat runnable jar whose start-class (main method  entry-point) is defined to be `io.pivotal.microservices.services.Main`.  This application takes a single argument that tells it to run as any of our three servers.

```
java -jar target/microservice-demo-1.2.0.RELEASE.jar registration|accounts|web
```
...
To run the microservices system from the command-line, open three CMD windows (Windows) or three Terminal windows (MacOS, Linux) and arrange so you can view them conveniently.
 ...
 1. In the same window run: `java -jar target/microservice-demo-1.2.0.RELEASE.jar registration`
 1. Switch to the second window and run: `java -jar target/microservice-demo-1.2.0.RELEASE.jar accounts`
 1. In the third window run: `java -jar target/microservice-demo-1.2.0.RELEASE.jar web`
...
As you interact you should logging in the second and third windows.
 ...
 1. In a new window, run up a second account-server using HTTP port 2223:
     * `java -jar target/microservice-demo-1.2.0.RELEASE.jar accounts 2223`
